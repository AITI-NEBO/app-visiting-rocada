...
function getMainKeyboard() {
    ...
    if (Option::get($moduleId, 'deal_stage_filter_tomorrow', '')) {
        $keyboard['keyboard'][0][] = ['text' => $cmd['visitsTomorrow']];
    }

    // Добавляем новую кнопку для планирования визита в том же ряду
    $keyboard['keyboard'][0][] = ['text' => $cmd['planVisit']];
...
function sendDealsList($telegramBot, $chatId, $page, $moduleId, $user, $callbackQuery = null, $tomorrow = false) {
	...
	if ($user['ID'] == 7951) unset($dealFilter['ASSIGNED_BY_ID']);
...
function backToDeal($dealId) {
	...
	$url = 'https://office.rocadatech.ru/local/modules/rocada.telegram/lib/app/full/?id=D_'.$deal['ID'];
    $kk = [
        'inline_keyboard' => [
            [
                [
    ...
    $telegramBot->sendMessage($chatId, "🎉 *Попробуйте обработать визиты в мини-приложении Telegram!*", $kk);
}
...
if ($chatId) {
    ...
    if ($text == $cmd['visits'] || $text == $cmd['visitsTomorrow'] || strpos($callbackData, 'page_') === 0) {
    ...
    } else if ($text == $cmd['planVisit']) {
        // Добавлена новая команда для планирования визита
        $messageText = "Заполните визит в мини-приложении:";
        $inlineKeyboard = [
            'inline_keyboard' => [
                [
                    [
                        ...
                        'web_app' => [
                            'url' => 'https://office.rocadatech.ru/local/modules/rocada.telegram/lib/app/new-visit/?'
                        ]
                    ]
                ]
            ]
        ];
        $telegramBot->sendMessage($chatId, $messageText, $inlineKeyboard);
        ...
        if ($record) {
            ...
            $telegramBot->sendMessage($chatId, 'Укажите причину завершения: ', $keyboard);
        }
    } else if ($text == "✅ Визит состоялся") {
        if(Option::get($moduleId,  'deal_stage_filter_win', '')) {
            if(count(json_decode(Option::get($moduleId,  'deal_stage_filter_win', ''))) == 1) {
                // Сбрасываем состояние пользователя, отправляем ссылку и перебрасываем в список сделок
                $record = $hlHandler->getUserRecord($isAuthorized['ID']);
                $hlHandler->clearUserRecord($isAuthorized['ID']);
                if($record && $record['UF_INFO']) {
                    DealTable::update($record['UF_INFO'], [
                        "STAGE_ID" => explode('|', json_decode(Option::get($moduleId,  'deal_stage_filter_win', ''))[0])[0]
                    ]);
                    $telegramBot->sendMessage($chatId, 'Укажите статусы завершения по ссылке - https://office.rocadatech.ru/crm/kasatka/?id=D\_' . $record["UF_INFO"], [
                        'inline_keyboard' => [
                            [['text' => 'Заполнить сейчас', 'web_app' => [
                                'url' => 'https://office.rocadatech.ru/local/modules/rocada.telegram/lib/app/result/?id=D\_' . $record["UF_INFO"]
                            ]]]
                        ]
                    ]);
                    $telegramBot->sendMessage($chatId, 'Главное меню', getMainKeyboard());
                }
                sendDealsList($telegramBot, $chatId, $page, $moduleId, $isAuthorized);
            } else {
                $inlineKeyboard = ['inline_keyboard' => []];
                foreach (json_decode(Option::get($moduleId,  'deal_stage_filter_win', '')) as $field) {
                    $inlineKeyboard['inline_keyboard'][] = [
                        ['text' => explode("|", $field)[1], 'callback_data' => "good_stage___".explode("|", $field)[0]]
                    ];
                }
                $telegramBot->sendMessage($chatId, "Перенести в стадию:", $inlineKeyboard);
            }
        }
    } else if($text == "❌ Сделка проиграна") {
        if(Option::get($moduleId,  'deal_stage_filter_lose', '')) {
            if(count(json_decode(Option::get($moduleId,  'deal_stage_filter_lose', ''))) == 1) {
                // Сбрасываем состояние пользователя, отправляем ссылку и перебрасываем в список сделок
                $record = $hlHandler->getUserRecord($isAuthorized['ID']);
                $hlHandler->clearUserRecord($isAuthorized['ID']);
                if($record && $record['UF_INFO']) {
                    DealTable::update($record['UF_INFO'], [
                        "STAGE_ID" => explode('|', json_decode(Option::get($moduleId,  'deal_stage_filter_lose', ''))[0])[0]
                    ]);
                    $telegramBot->sendMessage($chatId, "Сделка проиграна", getMainKeyboard());
                }
                sendDealsList($telegramBot, $chatId, $page, $moduleId, $isAuthorized);
            } else {
                $inlineKeyboard = ['inline_keyboard' => []];
                foreach (json_decode(Option::get($moduleId,  'deal_stage_filter_lose', '')) as $field) {
                    $inlineKeyboard['inline_keyboard'][] = [
                        ['text' => explode("|", $field)[1], 'callback_data' => "bad_stage___".explode("|", $field)[0]]
                    ];
                }
                $telegramBot->sendMessage($chatId, "Перенести в стадию:", $inlineKeyboard);
            }
        }
    } else if (strpos($callbackData, 'bad_stage_') === 0) {
        $record = $hlHandler->getUserRecord($isAuthorized['ID']);
        $hlHandler->clearUserRecord($isAuthorized['ID']);
        if($record && $record['UF_INFO']) {
            DealTable::update($record['UF_INFO'], [
                "STAGE_ID" => explode('___', $callbackData)[1]
            ]);
            $telegramBot->sendMessage($chatId, "Сделка провалена", getMainKeyboard());
        }
        sendDealsList($telegramBot, $chatId, $page, $moduleId, $isAuthorized);
    } else if (strpos($callbackData, 'good_stage_') === 0) {
        $record = $hlHandler->getUserRecord($isAuthorized['ID']);
        $hlHandler->clearUserRecord($isAuthorized['ID']);
        if($record && $record['UF_INFO']) {
            DealTable::update($record['UF_INFO'], [
            ...
            $telegramBot->sendMessage($chatId, "Встреча состоялась", getMainKeyboard());
        }
        sendDealsList($telegramBot, $chatId, $page, $moduleId, $isAuthorized);
    } else if ($location) {
        ...
        if ($record && $record['UF_STAGE'] === 'location_requested') {
            ...
            // Кнопки для возвращения назад и завершения сделки/встречи
            $keyboard = [
                'keyboard' => [
                    [
                        ['text' => "🔚 Завершить визит"],
            ...
            $telegramBot->sendMessage($chatId, "✅ *Геолокация успешно обновлена для сделки!*", $keyboard);
        } else {
