...
if ($request->isPost() && check_bitrix_sessid()) {
    ...
    Option::set($moduleId, 'deal_stage_filter_win', json_encode($request['deal_stage_filter_win']));
    Option::set($moduleId, 'deal_stage_filter_lose', json_encode($request['deal_stage_filter_lose']));
...
$selectedStageWin = Option::get($moduleId, 'deal_stage_filter_win', '');

if ($selectedStageWin !== null) {
    $selectedStageWin = json_decode(Option::get($moduleId, 'deal_stage_filter_win', ''));
} else {
    $selectedStageWin = [];
}
$selectedStageLose = Option::get($moduleId, 'deal_stage_filter_lose', '');

if ($selectedStageLose !== null) {
    $selectedStageLose = json_decode(Option::get($moduleId, 'deal_stage_filter_lose', ''));
} else {
    $selectedStageLose = [];
}
...
<form method="POST">
    ...
    <div id="settings">
        ...
        <div class="field-block">
            ...
            <div class="field">
                <label for="deal_stage_filter_win">Стадии для успешного закрытия:</label>
                <select name="deal_stage_filter_win[]" id="deal_stage_filter_win" multiple="true">
                    <?php foreach ($stagesList as $stageCode => $stageName): ?>
                        <option value="<?= $stageCode.'|'.$stageName ?>" <?= in_array($stageCode.'|'.$stageName, $selectedStageWin) ? 'selected' : '' ?>>
                            <?= $stageName ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="field">
                <label for="deal_stage_filter_lose">Стадии для провалившихся сделок:</label>
                <select name="deal_stage_filter_lose[]" id="deal_stage_filter_lose" multiple="true">
                    <?php foreach ($stagesList as $stageCode => $stageName): ?>
                        <option value="<?= $stageCode.'|'.$stageName ?>" <?= in_array($stageCode.'|'.$stageName, $selectedStageLose) ? 'selected' : '' ?>>
                            <?= $stageName ?>
                        </option>
                    <?php endforeach; ?>
                </select>
