    ...
    <div class="page-content">
      ...
      <section class="greeting-section animate-fade-in-up">
        <div class="greeting-text">
          ...
          <p class="greeting-role">{{ user.position }}</p>
        </div>
      </section>
        ...
        <div class="stats-card">
          ...
          <div class="stats-progress">
            ...
            <div class="progress-labels">
              <span class="progress-done">{{ user.stats.todayCompleted }} выполнено</span>
              <span class="progress-total">из {{ user.stats.todayTotal }}</span>
            </div>
          </div>
          <div class="stats-grid">
            <div class="stat-item">
              <span class="stat-value text-gradient">{{ user.stats.todayCompleted }}</span>
              <span class="stat-label">Завершено</span>
            </div>
            <div class="stat-item">
              <span class="stat-value" style="color: var(--color-warning)">{{ inProgress }}</span>
              <span class="stat-label">В работе</span>
            </div>
            <div class="stat-item">
              <span class="stat-value" style="color: var(--color-text-secondary)">{{ pending }}</span>
        ...
        <div class="actions-grid">
          ...
          <router-link to="/profile" class="action-card">
            <div class="action-icon-wrap" style="background: rgba(124, 58, 237, 0.12)">
              <span class="material-symbols-rounded action-icon" style="color: #A78BFA">bar_chart</span>
            </div>
            <span class="action-label">Стати-<br/>стика</span>
...
import { currentUser, visits, visitsTomorrow } from '../data/mock'

const user = currentUser
const todayCount = visits.length
const tomorrowCount = visitsTomorrow.length

const inProgress = computed(() => visits.filter(v => v.status === 'in_progress').length)
const pending = computed(() => visits.filter(v => v.status === 'pending').length)
const progressPercent = computed(() => Math.round((user.stats.todayCompleted / user.stats.todayTotal) * 100))

const nextVisit = computed(() => visits.find(v => v.status === 'in_progress' || v.status === 'pending'))
